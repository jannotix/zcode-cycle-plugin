...
- `1.0.10` 是当前生产版本。它关闭了首次隔离实机认证在 `1.0.9` 中发现的缺陷：只读取
...
- `1.0.8` 已发布，现已被替代。它之所以存在，是因为在为 `1.0.7` 准备认证期间认证主机
  ...
  威胁模型仍带有 `1.0.9` 已更正的那条表述。请改为安装 `1.0.10`。
- `1.0.7` 已发布，且在从未被认证之前即被替代。它的七项修复逐字节保留在 `1.0.10` 中。
  请改为安装 `1.0.10`。
- `1.0.6` 已发布，现已被替代。十三项中十一项通过；失败的两项——按角色指派模型的实际
  派发，以及被强行中断后的恢复——已在 `1.0.10` 关闭，同时关闭的还有项目身份分裂、首次
  运行的死锁、对路径视而不见的风险路由、一处文档错误，以及一道本应拦截却放行了自己
  要拦截的守护进程的发布门禁。请勿优先于 `1.0.10` 安装。
...
生产用户应仅在 `1.0.10` 被接纳并发布后，从 ZCode 官方公共插件市场安装。角色配置、
