...
import { LocalControlPlane, resolveDataDirectory, } from "./client.js";
...
const dataDirectory = process.env.ZCODE_CYCLE_DATA_DIR ?? resolveDataDirectory(process.platform, process.env);
...
const browserManager = new BrowserManager({
    ...
    create: (input) => new ManagedBrowserSessionFactory({
...
async function callTool(name, rawArgs) {
    ...
    switch (name) {
        case "cycle_health":
            return plane.health();
        case "cycle_start": {
            const preference = args.preference === "quick" || args.preference === "full" || args.preference === "auto"
                ? args.preference
                : undefined;
...
async function main() {
    ...
    process.stdin.on("close", () => {
        void Promise.allSettled([plane.dispose(), browserManager.dispose()]).finally(() => process.exit(0));
    });
}
void main();
