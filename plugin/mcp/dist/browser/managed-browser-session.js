...
import { isLoopback, } from "./browser-manager.js";
export class ManagedBrowserSessionFactory {
    ...
    async create(input) {
        ...
        return new PuppeteerBrowserSession(browser, page, origins, evidenceDirectory, profileDirectory, this.#options.projectDirectory);
    }
}
class PuppeteerBrowserSession {
    #actions = [];
    #browser;
    #evidenceDirectory;
    #logs = [];
    ...
    #redactions = new Set();
    #closed = false;
    constructor(browser, page, origins, evidenceDirectory, profileDirectory, projectDirectory) {
        ...
        page.on("requestfailed", (request) => this.#recordLog("requestfailed", "error", `${safeUrl(request.url())} ${request.failure()?.errorText ?? "unknown failure"}`));
    }
    allowOrigin(origin) {
    ...
    async run(command) {
        if (this.#closed)
            throw new Error("Managed browser session is closed");
        let result;
        switch (command.operation) {
            ...
            case "upload": {
                ...
                await (await target(this.#page, command)).uploadFile(path);
    ...
    async close() {
        ...
        try {
            ...
            await writeFile(receiptPath, `${JSON.stringify({ actions: this.#actions, logs: this.#logs }, null, 2)}\n`, { encoding: "utf8", flag: "wx" });
        }
        finally {
    ...
    async #check(command) {
        ...
        const element = await optionalTarget(this.#page, command);
        if (element !== undefined)
            assertions.visible = await visible(element);
        if (command.expectedText !== undefined) {
            const actual = this.#redact(element === undefined
                ? await this.#page.$eval("body", (body) => body.innerText)
        ...
        if (failed.length > 0)
            throw new Error(`Browser check failed: ${failed.join(", ")}`);
...
async function target(page, command) {
...
async function optionalTarget(page, command) {
    const selector = selectorFor(command);
    if (selector === undefined && !hasSemanticTarget(command))
        return undefined;
...
function hasSemanticTarget(command) {
...
async function semanticTarget(page, command) {
    const handle = await page.evaluateHandle(({ exact, name, role, text }) => {
        ...
        const accessibleName = (element) => {
            ...
            if (aria)
                return normalized(aria);
            if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
                const labels = [...(element.labels ?? [])].map((item) => item.textContent ?? "").join(" ");
                if (normalized(labels))
                    return normalized(labels);
                if (element.placeholder)
                    return normalized(element.placeholder);
                if (element instanceof HTMLInputElement &&
                    ["button", "reset", "submit"].includes(element.type)) {
                    return normalized(element.value);
                }
            }
            return normalized(element.getAttribute("alt") ?? element.getAttribute("title") ?? element.textContent);
        };
        return ([...document.querySelectorAll("body *")].find((element) => {
            if (role !== undefined && implicitRole(element) !== role)
                return false;
            if (name !== undefined && !matches(accessibleName(element), name))
                return false;
            if (text !== undefined && !matches(element.textContent ?? "", text))
                return false;
            return true;
        }) ?? null);
    }, {
...
function fillValue(command) {
    ...
    if (command.environmentVariable === undefined)
    ...
    if (value === undefined)
        throw new Error("Browser credential environment variable is not set");
    return value;
}
export async function resolveBrowserExecutable(configured) {
...
function truncate(value, maximum = 60_000) {
