...
export class LocalControlPlane {
    ...
    async audit(observation) {
        ...
        try {
            ...
            if (message.type === "error") {
                ...
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the audit event");
            }
            if (message.type !== "audit_recorded") {
                throw new ControlPlaneError("workflowd returned an unexpected audit response");
            }
            const data = asRecord(message.data);
            if (data.request_id !== 2 ||
    ...
    async history(projectKey, operation) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the history request");
            }
    ...
    async control(projectKey, operation, workflowId) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the control request");
            }
    ...
    async admission(projectKey, workflowId, workspace, operation) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string"
                    ? data.message
    ...
    async codeIndex(projectKey, workflowId, projectDirectory) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the code index");
            }
    ...
    async memory(projectKey, operation) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the memory request");
            }
    ...
    async prepareWorktree(projectKey, projectDirectory, workflowId) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the worktree");
            }
            if (message.type !== "worktree") {
                throw new ControlPlaneError("workflowd returned an unexpected worktree response");
            }
            const data = asRecord(message.data);
            if (data.request_id !== 7 ||
                data.workflow_id !== workflowId ||
    ...
    async freezeCandidate(projectKey, workflowId, baseRevision, planId, evidenceIds, candidateId = randomUUID()) {
        ...
        try {
            ...
            if (message.type === "error") {
                const data = asRecord(message.data);
                throw new ControlPlaneError(typeof data.message === "string" ? data.message : "workflowd rejected the candidate");
            }
            if (message.type !== "candidate_frozen") {
                throw new ControlPlaneError("workflowd returned an unexpected candidate response");
            }
            const data = asRecord(message.data);
            if (data.request_id !== 8 ||
                data.workflow_id !== workflowId ||
    ...
    async verifyCandidate(projectKey, workflowId, candidateId, planId, attestations = []) {
        ...
        if (attestations.length > 32 ||
            attestations.some((attestation) => !SHA256.test(attestation.candidate_digest) ||
    ...
    async #exchange(requestId, requestType, responseType, data, timeoutMillis = 10_000) {
        const secret = await readSecret(this.#secretPath);
        if (secret === undefined)
            throw new ControlPlaneError("workflowd credential disappeared");
        const { decoder, socket } = await this.#connect(secret);
...
export function resolveDataDirectory(platform, environment) {
...
function readJson(socket, decoder, timeoutMillis = 10_000) {
    return new Promise((resolve, reject) => {
        const cleanup = () => {
            clearTimeout(timeout);
...
function parseChallenge(value) {
    ...
    if (!Number.isSafeInteger(challenge.expires_at_unix_millis) ||
...
function calculateMac(secret, challenge) {
    ...
    return Array.from(createHmac("sha256", secret)
        .update(AUTH_DOMAIN)
...
function parseHealth(value) {
    ...
    if (typeof report.product_version !== "string" ||
...
function parseCandidateManifest(value) {
    ...
    if (typeof manifest.candidate_id !== "string" ||
