...
- 卸载移除的是当前安装，而不是市场的缓存副本。ZCode 会在你的 ZCode 配置目录下的市场
  缓存中保留已下载的插件——约 76 MB，含每个受支持平台的原生守护进程——而其确认对话框
  并未说明这一点。该副本是惰性的：它不在 `installed_plugins.json` 中，因此不会被加载，
  也不会有守护进程从中运行。若要回收这部分空间，请在卸载插件后再删除该市场本身。
