...
- `1.0.4` 是尚未发布的生产版本。在同一个不可变插件制品完成 Windows/Linux
  发布矩阵之前，不得分发。
...
生产用户应仅在 `1.0.4` 被接纳并发布后，从 ZCode 官方公共插件市场安装。角色配置、
...
`1.0.4` 内置的 `workflowd.exe` **未**经过 Authenticode 签名。首次使用时
...
签名将在后续版本中恢复。已发布版本不可变，因此 `1.0.4` 将始终保持未签名状态。
...
开发披露：为 `1.0.4` 准备的变更包含 AI 辅助生成的代码和文档，发布前必须由项目所有者
进行人工审查。
