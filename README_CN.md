...
- `1.0.8` 是当前生产版本。它与 `1.0.7` 携带完全相同的七项修复，未改动任何产品代码。
  它之所以存在，是因为在为 `1.0.7` 准备认证期间认证主机发生了变化：内置的 ZCode CLI
  从 `0.16.5` 升至 `0.16.9`，而该版本号被写在随产品分发的威胁模型中，因此已发布的
  `1.0.7` 归档描述的是一个已不复存在的主机配置。详见 [变更日志](CHANGELOG.md)。
- `1.0.7` 已发布，且在从未被认证之前即被替代。它的七项修复逐字节保留在 `1.0.8` 中，
  唯一的差别是它所声明的主机配置。请改为安装 `1.0.8`。
- `1.0.6` 已发布，现已被替代。十三项中十一项通过；失败的两项——按角色指派模型的实际
  派发，以及被强行中断后的恢复——已在 `1.0.8` 关闭，同时关闭的还有项目身份分裂、首次
  运行的死锁、对路径视而不见的风险路由、一处文档错误，以及一道本应拦截却放行了自己
  要拦截的守护进程的发布门禁。请勿优先于 `1.0.8` 安装。
...
生产用户应仅在 `1.0.8` 被接纳并发布后，从 ZCode 官方公共插件市场安装。角色配置、
...
`1.0.8` 内置的 `workflowd.exe` **未**经过 Authenticode 签名。首次使用时
...
签名将在后续版本中恢复。已发布版本不可变，因此 `1.0.8` 将始终保持未签名状态。
