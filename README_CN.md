...
- `1.0.7` 是当前生产版本。它是在 `1.0.6` 基础上关闭了六个缺陷——这些缺陷全部来自把
  `1.0.6` 自己已发布的归档在 Windows 与 Linux 上跑完十三项实机认证时发现的。详见
  [变更日志](CHANGELOG.md)。
- `1.0.6` 已发布，现已被替代。十三项中十一项通过；失败的两项——按角色指派模型的实际
  派发，以及被强行中断后的恢复——已在 `1.0.7` 关闭，同时关闭的还有项目身份分裂、首次
  运行的死锁、对路径视而不见的风险路由，以及一处文档错误。请勿优先于 `1.0.7` 安装。
...
生产用户应仅在 `1.0.7` 被接纳并发布后，从 ZCode 官方公共插件市场安装。角色配置、
...
`1.0.7` 内置的 `workflowd.exe` **未**经过 Authenticode 签名。首次使用时
...
签名将在后续版本中恢复。已发布版本不可变，因此 `1.0.7` 将始终保持未签名状态。
