# Cycle for Zcode 1.0.4 Production Release Plan
...
- Product: Cycle for Zcode `1.0.4`. `1.0.3` was sealed and certified live,
  and that campaign found three defects - two of them fixes from `1.0.3` that
  never ran, and a gate reporting a judgement it never made. `1.0.2` and the
...
- Windows/Linux ARM64: unsupported in `1.0.4`; the marketplace documentation
...
1. **Version and history** - `1.0.0` is marked withdrawn and `1.0.1` is
   marked superseded; all final installable manifests say `1.0.4`; historical
   tags are unchanged.
...
8. **Live ZCode** - clean install from the final ZIP, component discovery,
   ...
   gates for `1.0.4` and cannot be: `v1.0.0` was withdrawn carrying no release
   ...
   certified at the first version with a genuinely published predecessor.
